...
for (i in 1:length(window_sizes)) {
  smoothed_plot <- ggplot(kendall_smoothed_results[[i]], aes(x = as.Date(paste0(week, "-1"), format = "%Y-%U-%u"), y = smoothed_attacks)) +
    ...
    labs(title = paste("Згладжена частота атак по тижнях у 2024 році (Kendall, W =", window_sizes[i], ")"),
         x = "Тиждень",
         y = "Згладжена кількість атак") +
    theme_minimal()
  
  # Save the smoothed plot to a file
  ggsave(paste0("kendall_smoothed_attacks_2024_W_", window_sizes[i], ".png"), plot = smoothed_plot)
...
for (i in 1:length(alpha_values)) {
  smoothed_plot <- ggplot(exponential_smoothed_results[[i]], aes(x = as.Date(paste0(week, "-1"), format = "%Y-%U-%u"), y = smoothed_attacks)) +
    ...
    labs(title = paste("Згладжена частота атак по тижнях у 2024 році (Exponential, alpha =", alpha_values[i], ")"),
         x = "Тиждень",
         y = "Згладжена кількість атак") +
    theme_minimal()
  
  # Save the smoothed plot to a file
  ggsave(paste0("exponential_smoothed_attacks_2024_alpha_", alpha_values[i], ".png"), plot = smoothed_plot)
...
for (i in 1:length(window_sizes)) {
  smoothed_plot <- ggplot(pollard_smoothed_results[[i]], aes(x = as.Date(paste0(week, "-1"), format = "%Y-%U-%u"), y = smoothed_attacks)) +
    ...
    labs(title = paste("Згладжена частота атак по тижнях у 2024 році (Pollard, W =", window_sizes[i], ")"),
         x = "Тиждень",
         y = "Згладжена кількість атак") +
    theme_minimal()
  
  # Save the smoothed plot to a file
  ggsave(paste0("pollard_smoothed_attacks_2024_W_", window_sizes[i], ".png"), plot = smoothed_plot)
...
for (i in 1:length(window_sizes)) {
  smoothed_plot <- ggplot(median_filtered_results[[i]], aes(x = as.Date(paste0(week, "-1"), format = "%Y-%U-%u"), y = smoothed_attacks)) +
    ...
    labs(title = paste("Згладжена частота атак по тижнях у 2024 році (Median, W =", window_sizes[i], ")"),
         x = "Тиждень",
         y = "Згладжена кількість атак") +
    theme_minimal()
  
  # Save the smoothed plot to a file
  ggsave(paste0("median_filtered_attacks_2024_W_", window_sizes[i], ".png"), plot = smoothed_plot)
